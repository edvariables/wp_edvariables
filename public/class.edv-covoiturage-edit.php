...
class edv_Covoiturage_Edit {
	...
	public static function init_hooks() {
		...
		add_action( 'wp_ajax_'.EDV_TAG.'_'.EDV_COVOIT_SECRETCODE, array(__CLASS__, 'on_wp_ajax_covoiturage_code_secret_cb') );
		add_action( 'wp_ajax_nopriv_'.EDV_TAG.'_'.EDV_COVOIT_SECRETCODE, array(__CLASS__, 'on_wp_ajax_covoiturage_code_secret_cb') );
	}
 	/////////////
	...
	*/
	public static function get_post($covoiturage_id = false) {
		return edv_Covoiturage::get_post($covoiturage_id);
	}
	...
	*/
	public static function check_nonce() {
		foreach([EDV_TAG . '-' . EDV_COVOIT_SECRETCODE,
	...
	public static function get_post_title( ) {
 		if( $post = self::get_post()){
			...
			if($post_id){
				...
				$html = edv_Covoiturage::get_covoiturage_dates_text( $post_id )
					. '<br>' . $post_title;
				return $html;
			}
		}
	...
	public static function get_default_value( $field_name ) {
		...
		switch($field_name){
			...
			case 'cov-' . EDV_COVOIT_SECRETCODE :
				return edv::get_secret_code(4, 'num');
	...
	public static function get_covoiturage_edit_content( ) {
		...
		$form_id = edv::get_option('covoiturage_edit_form_id');
		if(!$form_id){
			return edv::icon('warning', '', 'edv-error-light'
				, 'Le formulaire de modification du covoiturage n\'est pas défini dans les réglages de edv.', 'div');
		}
		...
		$duplicate_from_id = false;
 		if( ! $post && array_key_exists('action', $_GET) ){
			...
			&& array_key_exists(EDV_ARG_COVOITURAGEID, $_GET)){
				$duplicate_from_id = $_GET[EDV_ARG_COVOITURAGEID];
 		...
 		if( $post ){
 			$post_id = $post->ID;
			if( ! edv_Covoiturage::user_can_change_post($post)){
			...
			foreach(['cov-date-debut',
					...
					'cov-phone',
					'cov-phone-show',
					'cov-' . EDV_COVOIT_SECRETCODE,
					'cov-organisateur',
					'cov-nb-places'
			] as $meta_name){
				$attrs[$meta_name] = edv_Covoiturage::get_post_meta($post_id, $meta_name, true, false);
			}
		}
		else{
		...
		if($duplicate_from_id){
			$title = edv_Covoiturage::get_post_title($post, true);
			$url = edv_Covoiturage::get_post_permalink( $post_id, EDV_COVOIT_SECRETCODE);
		...
		elseif($post_id){
			...
			$ekey = edv_Covoiturage::get_secretcode_in_request($post_id);		
			if($ekey){
				$input .= sprintf('<input type="hidden" name="%s" value="%s"/>', EDV_COVOIT_SECRETCODE, $ekey);
			}
		}
	...
	public static function get_edit_toolbar($post){
		...
		$url = add_query_arg(EDV_ARG_COVOITURAGEID, $post_id, $url );
		$html .= sprintf('<span class="covoiturage-tool"><a href="%s" title="%s">%s%s</a></span>'
				, esc_url($url)
				, __('Ecrivez-nous pour signaler un problème avec ce covoiturage', EDV_TAG)
				, edv::icon('email-alt')
				, __('Un problème ?', EDV_TAG)
		);
				
		if($post->post_status == 'publish')
			$html .= sprintf('<span class="covoiturage-tool">%s</span>', edv_Covoiturage::get_covoiturage_action_link($post_id, 'unpublish', true));
		elseif( current_user_can('manage_options')
		|| (! edv_Covoiturage::waiting_for_activation($post_id)
			&& edv_Covoiturage::user_can_change_post($post_id))){
			$html .= sprintf('<span class="covoiturage-tool">%s</span>', edv_Covoiturage::get_covoiturage_action_link($post_id, 'publish', true));
		}
		if(current_user_can('manage_options')
		|| current_user_can('covoiturage')
		|| edv_Covoiturage::user_can_change_post($post_id))
			$html .= sprintf('<span class="covoiturage-tool">%s</span>', edv_Covoiturage::get_covoiturage_action_link($post_id, 'duplicate', true));
		$html .= sprintf('<span class="covoiturage-tool">%s</span>', edv_Covoiturage::get_covoiturage_action_link($post_id, 'remove', true));
		$html .= sprintf('<span class="covoiturage-tool">%s</span>', edv_Covoiturage::get_covoiturage_contact_email_link($post_id, true));
		$html .= '</div>';
	 ...
	 */
 	public static function wpcf7_form_init_tags_cb( $form_class ) { 
		...
		foreach( edv_Covoiturage_Post_type::get_taxonomies() as $tax_name => $taxonomy){
			...
			$all_terms = edv_Covoiturage_Post_type::get_all_terms($tax_name);
			$checkboxes = '';
			$selected = '';
			$free_text = false;
			$index = 0;
			...
			switch($tax_name){
				case edv_Covoiturage::taxonomy_city :
					// $checkboxes .= '"(autre)|0"';
					// $free_text = 'free_text';
					break;
	...
	private static function get_covoiturage_edit_content_forbidden( $post ) {
		...
		if($post->post_status == 'trash'){
			...
			$html .= '<br>' . edv_Covoiturage::get_covoiturage_contact_email_link($post, true);
			
			//Formulaire de saisie du code secret
			$url = edv_Covoiturage::get_post_permalink( $post );
			$query = [
				'post_id' => $post_id,
				'action' => EDV_TAG . '_' . EDV_COVOIT_SECRETCODE
			];
			$html .= sprintf('<br>Vous connaissez le code secret de ce covoiturage :&nbsp;'
				. '<form class="edv-ajax-action" data="%s">'
				. wp_nonce_field(EDV_TAG . '-' . EDV_COVOIT_SECRETCODE, EDV_TAG . '-' . EDV_COVOIT_SECRETCODE, true, false)
				.'<input type="text" placeholder="ici le code" name="'.EDV_COVOIT_SECRETCODE.'" size="7"/>
	...
	public static function on_wp_ajax_covoiturage_code_secret_cb() {
		...
		if(array_key_exists("post_id", $_POST)){
			...
			if($post->post_type != edv_Covoiturage::post_type)
				return;
			$input = $_POST[EDV_COVOIT_SECRETCODE];
			$codesecret = edv_Covoiturage::get_post_meta($post, 'cov-' . EDV_COVOIT_SECRETCODE, true);
			if(strcasecmp( $codesecret, $input) == 0){
				//TODO : transient plutot que dans l'url
				$url = edv_Covoiturage::get_post_permalink($post, EDV_COVOIT_SECRETCODE . '=' . $codesecret);
	...
	public static function submit_covoiturage_form($contact_form, &$abort, $submission){
		...
		if( ! array_key_exists('covoiturage_duplicated_from', $_POST)){
			...
			elseif( ! edv_Covoiturage::user_can_change_post($post)){
				$abort = true;
		...
		// $meta_name = 'cov-'.EDV_COVOIT_SECRETCODE;
		...
		$new_post_need_validation = edv::get_option('covoiturage_need_validation', false);
		if( ! $post && ! $post_author ){
			$data['activation_key'] = $new_post_need_validation;
		}
		
		$post_title = edv_Covoiturage::get_post_title($post, true, $data);
		...
		if( $post_is_new && ! is_user_logged_in()){
			if( $data['cov-email']) {
				$result = edv_Covoiturage::send_validation_email($post_id, false, false, 'bool');
		...
		&& edv_Covoiturage::waiting_for_activation($post)){
			$result = edv_Covoiturage::send_validation_email($post_id, false, false, 'bool');
	...
	*/
	...
	public static function get_post_idem($post_title, $meta_values){
		...
		$args = edv_Covoiturages::get_posts_query( 
			array(
