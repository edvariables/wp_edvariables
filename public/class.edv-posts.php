...
class edv_Posts {
	...
	public static function init_hooks() {
		add_action( 'wp_ajax_'.edv_Post::post_type.'_show_more', array(__CLASS__, 'on_wp_ajax_edposts_show_more_cb') );
		add_action( 'wp_ajax_nopriv_'.edv_Post::post_type.'_show_more', array(__CLASS__, 'on_wp_ajax_edposts_show_more_cb') );
		add_action( 'wp_ajax_'.EDV_TAG.'_edposts_action', array(__CLASS__, 'on_wp_ajax_edposts') );
		add_action( 'wp_ajax_nopriv_'.EDV_TAG.'_edposts_action', array(__CLASS__, 'on_wp_ajax_edposts') );
	}
	/*
	 * Hook
	 ******/
	...
	public static function get_url(){
		$url = get_permalink(edv::get_option('edposts_page_id')) . '#main';
		// $url = home_url();
		return $url;
	}
	...
	public static function get_posts_months(){
		...
		$sql = "SELECT DISTINCT DATE_FORMAT(meta.meta_value, '%Y') as year
				...
				INNER JOIN {$blog_prefix}postmeta meta
					...
					AND meta.meta_key = 'edp-date-debut'
					AND meta.meta_value >= CURDATE()
				WHERE posts.post_status = 'publish'
					AND posts.post_type = '". edv_Post::post_type ."'
		";
		if($sql_filters)
		...
		foreach($result as $row){
			if($predv_row)
				if($predv_row->year === $row->year){
					for($m = (int)$predv_row->month + 1; $m < (int)$row->month; $m++)
						$months[$predv_row->year . '-' . sprintf("%02d",$m)] = 0;
				}
				elseif((int)$predv_row->year === (int)$row->year - 1){
					for($m = (int)$predv_row->month + 1; $m <= 12; $m++)
						$months[$predv_row->year . '-' . $m] = 0;
			...
			if(count($query_tax_terms)){
				...
				foreach($query_tax_terms as $tax_name => $tax_data){
					...
					&& count($tax_data['IN'])){
						$sql .=   "\n INNER JOIN {$blog_prefix}postmeta as localisation"
									...
									. "\n AND localisation.meta_key = 'edp-localisation'"
						;
					}
				}
				//WHERE
				...
				foreach($query_tax_terms as $tax_name => $tax_data){
					if(count($tax_data['IN'])){
						...
						;
						if($tax_name === edv_Post::taxonomy_city){
							$terms_like = edv_Post_Post_type::get_terms_like($tax_name, $tax_data['IN']);
	...
	*
	* Optimal sous la forme https://.../edposts-local/?eventid=1207#eventid1207
	*/
	public static function get_list_html($content = '', $options = false){
		...
		$requested_id = array_key_exists(EDV_ARG_POSTID, $_GET) ? $_GET[EDV_ARG_POSTID] : false;
		$requested_month = false;
		if($requested_id){
			$date_debut = get_post_meta($requested_id, 'edp-date-debut', true);
			if($date_debut)
				$requested_month = substr($date_debut, 0, 7);
		}
		
		$html = sprintf('<div class="edv-edposts edv-edposts-%s">', $options['mode']);
		if( $options['mode'] != 'email')
		...
		foreach($months as $month => $month_events_count) {
			...
			&& $month !== $requested_month) {
				...
				$ajax = sprintf('ajax="once" data="%s"',
					esc_attr( json_encode ( array(
						'action' => edv_Post::post_type . '_show_more',
						'data' => $data
					)))
				);
			} else
			...
			$html .= sprintf(
				...
				<ul id="month-%s" class="edposts-month toggle-container">'
	...
	*/
	public static function get_list_for_email($content = '', $options = false){
		...
		if(edv_Post_Post_type::is_diffusion_managed()){
			...
			self::add_tax_filter(edv_Post::taxonomy_diffusion, $term_id);
		}
...
.edv-edposts-email .edpost .dates {
	font-size: larger;
	font-weight: bold;
} 
.edv-edposts-email a-li a-li {
	margin-left: 1em;
	padding-top: 2em;
}
.edv-edposts-email div.titre, .edv-edposts-email div.localisation, .edv-edposts-email div.edp-cities {
	font-weight: bold;
}
.edv-edposts-email i {
	font-style: normal;
	...
	*/
	public static function get_list_header($requested_month = false, $options = false){
		...
		if( ! current_user_can('manage_options') || edv::get_option('newsletter_diffusion_term_id') == -1)
			$except_tax = edv_Post::taxonomy_diffusion;
		else
			$except_tax = '';
		$taxonomies = edv_Post_Post_type::get_taxonomies($except_tax);
		foreach( $taxonomies as $tax_name => $taxonomy){
			$taxonomy['terms'] = edv_Post_Post_type::get_all_terms($tax_name);
		...
		$html = '<div class="edv-edposts-list-header">'
			...
			. '<p class="edv-title-link">'
				. '<a href="'. get_page_link( edv::get_option('new_post_page_id')) .'" title="Cliquez ici pour ajouter un nouvel message"><span class="dashicons dashicons-welcome-add-page"></span></a>'
	...
	*/
	public static function get_month_posts_list_html($month, $requested_id = false, $options = false){
		...
		if(is_wp_error( $events)){
			$html = sprintf('<p class="alerte no-events">%s</p>%s', __('Erreur lors de la recherche des messages.', EDV_TAG), var_export($events, true));
		}
		elseif($events){
			$html = '';
			if(count($events) === 0){
				$html .= sprintf('<p class="alerte no-events">%s</p>', __('Aucun message trouvé', EDV_TAG));
			}
			else {
	...
	public static function get_list_item_html($event, $requested_id, $options){
		...
		$date_debut = get_post_meta($event->ID, 'edp-date-debut', true);
					
		$url = edv_Post::get_post_permalink( $event );
		$html = '';
		...
		$html .= sprintf('<div id="%s%d" class="edpost toggle-trigger %s" edpost="%s">'
			, EDV_ARG_POSTID, $event->ID
			, $event->ID == $requested_id ? 'active' : ''
			, esc_attr( json_encode(['id'=> $event->ID, 'date' => $date_debut]) )
		);
		
		$cities = edv_Post::get_event_cities ($event, 'names');
		
		$value = $event->post_title;
		$localisation = htmlentities(get_post_meta($event->ID, 'edp-localisation', true));
		if($cities){
			$cities = htmlentities(implode(', ', $cities));
			if(self::cities_in_localisation( $cities, $localisation ) === false)
				$localisation .= sprintf('<div class="edp-cities" title="%s"><i>%s</i></div>', 'Communes', $cities);
		}
		$dates = edv_Post::get_event_dates_text($event->ID);
		$html .= sprintf(
		...
		$categories = edv_Post::get_event_categories ($event, 'names');
		// var_dump($categories); die();
		if($categories)
			$html .= sprintf('<div class="edp-categories" title="%s"><i>%s</i></div>', 'Catégories', htmlentities(implode(', ', $categories)));
		...
		$value = get_post_meta($event->ID, 'edp-organisateur', true);
		if($value){
			$html .= sprintf('<div>Organisé par : %s</div>',  htmlentities($value) );
		}
		
		$value = get_post_meta($event->ID, 'edp-phone', true);
		if($value){
			$html .= sprintf('<div class="edp-phone">Téléphone : %s</div>',  antispambot($value) );
		}
		
		$value = get_post_meta($event->ID, 'edp-siteweb', true);
		if($value){
			$html .= sprintf('<div class="edp-siteweb">%s</div>',  make_clickable( esc_html($value) ) );
		}
	 ...
	 * Show more
	 */
	public static function on_wp_ajax_edposts_show_more_cb() {
	...
	/* public static function on_wp_ajax_edposts_show_more_cb() {
		if(!array_key_exists("last-post", $_POST)){
			...
			$ajax_response = self::get_edposts_list_html($posts);
		}
	...
	public static function download_links(){
		...
		$title = 'Télécharger les messages au format ICS';
		if( count($_GET) && self::$filters_summary ){
			$title .= ' filtrés (' . self::$filters_summary . ')';
			$data['filters'] = $_GET;
		}
		$html .= edv::get_ajax_action_link(false, ['edposts','download_file'], 'download', '', $title, false, $data);
		
		$meta_name = 'download_link';
		foreach(edv_Post_Post_type::get_all_terms(edv_Post::taxonomy_diffusion) as $term_id => $term){
			$file_format = get_term_meta($term->term_id, $meta_name, true);
			if( $file_format ){
				$data = [
					'file_format' => $file_format,
					'filters' => [ edv_Post::taxonomy_diffusion => $term_id]
				];
				$title = sprintf('Télécharger les messages pour %s (%s)', $term->name, $file_format);
				$html .= edv::get_ajax_action_link(false, ['edposts','download_file'], 'download', '', $title, false, $data);
			}
		}
		return $html;
	 ...
	 * Requête Ajax
	 */
	public static function on_wp_ajax_edposts() {
		if( ! edv::check_nonce()
		|| empty($_POST['method']))
			wp_die();
	...
	public static function on_ajax_action_download_file($data) {
		...
		// $sql = "SELECT posts.*, date_debut.meta_value AS dt_debut, heure_debut.meta_value as h_debut"
			...
			// . "\n AND date_debut.meta_key = 'edp-date-debut'"
			...
			// . "\n AND heure_debut.meta_key = 'edp-heure-debut'"
			// . "\n WHERE posts.post_status = 'publish'"
		...
		if( ! $posts)
			return sprintf('Aucun message à exporter');
		
		$file_format = $data['file_format'];
		
		require_once( dirname(__FILE__) . '/class.edv-edposts-export.php');
		$url = edv_Posts_Export::do_export($posts, $file_format, 'url');
